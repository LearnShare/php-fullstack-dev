...
3. **[引用、箭头函数与闭包](section-03-references-closures.md)**：引用传参、箭头函数（PHP 7.4+）、闭包、变量捕获（按值、按引用）、闭包与类、`Closure::bindTo()`、变量生命周期、与 JavaScript 对比及完整示例。

4. **[匿名函数](section-04-anonymous-functions.md)**：匿名函数的定义、调用、类型声明、在数组函数中使用、立即执行函数（IIFE）、作为参数/返回值及完整示例。

5. **[可调用类型与内置函数](section-05-callable-types.md)**：可调用类型的各种形式、`is_callable()`、`call_user_func()`、`call_user_func_array()`、内置函数工具及完整示例。
