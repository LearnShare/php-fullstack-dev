...
1. **[2.7.1 字符串创建方式](section-01-string-creation.md)**：单引号、双引号、Heredoc、Nowdoc 的语法、特点、变量插值、转义序列及选择建议。

2. **[2.7.2 常用字符串函数](section-02-string-functions.md)**：长度计算（`strlen`、`mb_strlen`）、查找定位（`strpos`、`mb_strpos`）、截取（`substr`、`mb_substr`）、替换（`str_replace`）、去除空白（`trim`）、大小写转换、分割拼接、格式化等函数的详细用法。
